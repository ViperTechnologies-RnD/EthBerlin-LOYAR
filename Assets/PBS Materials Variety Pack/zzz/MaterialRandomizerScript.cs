...
public class MaterialRandomizerScript : MonoBehaviour {
  ...
  public void findMaterials() {
    string[] guids = AssetDatabase.FindAssets(
        "t:Material", new[] { "Assets/PBS Materials Variety Pack/" });
    materials.Clear();
    foreach (string id in guids) {
      // Debug.Log(AssetDatabase.GUIDToAssetPath(id));
      Material mat = AssetDatabase.LoadMainAssetAtPath(
          AssetDatabase.GUIDToAssetPath(id)) as Material;
  ...
  public void findMaterialSpheres() {
    gameObjects.Clear();
    foreach (var gameObj in FindObjectsOfType(typeof(GameObject))
                 as GameObject[]) {
